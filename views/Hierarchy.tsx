...
import { CompanyBranch, CompanyDepartment, Employee } from '../types';
...
export const INITIAL_HIERARCHY: CompanyBranch[] = [
  {
    ...
    departments: [
      { 
        ...
        supervisorId: 'emp1', 
        supervisorName: 'زيد الهاشمي', 
        employeeCount: 15,
        positions: ['مدير تقني', 'مطور برمجيات', 'مهندس شبكات', 'دعم فني']
      },
      { 
        id: 'd2', 
        name: 'المالية والمحاسبة', 
        supervisorId: 'emp3', 
        supervisorName: 'عمر القيسي', 
        employeeCount: 8,
        positions: ['محاسب أول', 'مدقق مالي', 'أمين صندوق']
      },
    ]
  },
  {
    ...
    managerId: 'emp2',
    managerName: 'مريم الجبوري',
    departments: [
      { 
        id: 'd3', 
        name: 'الموارد البشرية', 
        supervisorId: 'emp2', 
        supervisorName: 'مريم الجبوري', 
        employeeCount: 5,
        positions: ['مسؤول توظيف', 'أخصائي تدريب', 'منسق إداري']
      },
      { 
        id: 'd4', 
        name: 'المبيعات والتسويق', 
        supervisorId: 'emp4', 
        supervisorName: 'نور السعدي', 
        employeeCount: 20,
        positions: ['مدير مبيعات', 'مندوب تسويق', 'مصمم جرافيك']
      },
    ]
  }
];
...
const Hierarchy: React.FC<HierarchyProps> = ({ hierarchy, setHierarchy, employees }) => {
    ...
    <div className="space-y-6 pb-20">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500">إدارة فروع الشركة وأقسامها وتحديد المسؤوليات الإدارية والمسميات الوظيفية</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 gap-8">
        {hierarchy.map((branch) => (
          <div key={branch.id} className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {branch.departments.map((dept) => (
                  <div key={dept.id} className="group border border-slate-100 rounded-xl p-5 hover:border-blue-200 hover:shadow-md transition-all relative flex flex-col h-full">
                    ...
                    <div className="text-sm text-slate-500 space-y-2 mb-4">
                       <div className="flex items-center">
                          <span className="w-20 font-medium">المسؤول:</span>
                          <span className="text-blue-600 font-bold">{dept.supervisorName}</span>
                       </div>
                    </div>

                    <div className="bg-slate-50 rounded-lg p-3 flex-grow">
                      <p className="text-xs font-bold text-slate-400 mb-2 uppercase tracking-wider">المسميات الوظيفية</p>
                      <div className="flex flex-wrap gap-2 mb-3">
                        {dept.positions.map((pos) => (
                          <span key={pos} className="inline-flex items-center bg-white border border-slate-200 text-slate-700 px-2 py-1 rounded text-[10px] font-bold">
                            {pos}
                      ...
                      {positionEditingDeptId === dept.id ? (
                        <div className="flex gap-2">
                          <input 
                            ...
                            autoFocus
                          />
                          <button onClick={() => handleAddPosition(branch.id, dept.id)} className="bg-blue-600 text-white p-1 rounded"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7"/></svg></button>
                        </div>
                      ) : (
                        <button 
                ...
                <button 
                  onClick={() => handleAddDept(branch.id)}
                  className="border-2 border-dashed border-slate-200 rounded-xl p-5 flex flex-col items-center justify-center text-slate-400 hover:border-blue-300 hover:text-blue-400 hover:bg-blue-50/30 transition-all min-h-[200px]"
                >
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between">
              <h3 className="text-xl font-bold">{editingBranchId ? 'تعديل بيانات الفرع' : 'إضافة فرع جديد'}</h3>
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-1">اسم الفرع *</label>
                <input type="text" className="w-full border-slate-200 rounded-lg p-2.5" placeholder="مثلاً: فرع البصرة" value={branchForm.name} onChange={e => setBranchForm({...branchForm, name: e.target.value})} />
              </div>
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-1">الموقع الجغرافي</label>
                <input type="text" className="w-full border-slate-200 rounded-lg p-2.5" placeholder="أدخل العنوان التفصيلي" value={branchForm.location} onChange={e => setBranchForm({...branchForm, location: e.target.value})} />
              </div>
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-1">مدير الفرع المسئول</label>
                <select className="w-full border-slate-200 rounded-lg p-2.5" value={branchForm.managerId} onChange={e => setBranchForm({...branchForm, managerId: e.target.value})}>
            ...
            <div className="p-6 border-t border-slate-100 flex space-x-3 space-x-reverse">
              <button onClick={handleSaveBranch} className="flex-1 bg-blue-600 text-white py-2.5 rounded-xl font-bold hover:bg-blue-700">
                {editingBranchId ? 'حفظ التعديلات' : 'حفظ الفرع'}
              </button>
              <button onClick={() => setIsBranchModalOpen(false)} className="px-6 py-2.5 bg-slate-100 text-slate-600 rounded-xl font-bold">إلغاء</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isDeptModalOpen && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between">
              <h3 className="text-xl font-bold">{editingDeptId ? 'تعديل بيانات القسم' : 'إضافة قسم جديد'}</h3>
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-1">اسم القسم *</label>
                <input type="text" className="w-full border-slate-200 rounded-lg p-2.5" placeholder="مثلاً: قسم المشتريات" value={deptForm.name} onChange={e => setDeptForm({...deptForm, name: e.target.value})} />
              </div>
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-1">المسؤول المباشر</label>
                <select className="w-full border-slate-200 rounded-lg p-2.5" value={deptForm.supervisorId} onChange={e => setDeptForm({...deptForm, supervisorId: e.target.value})}>
            ...
            <div className="p-6 border-t border-slate-100 flex space-x-3 space-x-reverse">
              <button onClick={handleSaveDept} className="flex-1 bg-blue-600 text-white py-2.5 rounded-xl font-bold hover:bg-blue-700">
                {editingDeptId ? 'حفظ التعديلات' : 'حفظ القسم'}
              </button>
              <button onClick={() => setIsDeptModalOpen(false)} className="px-6 py-2.5 bg-slate-100 text-slate-600 rounded-xl font-bold">إلغاء</button>
            </div>
          </div>
        </div>
      )}
